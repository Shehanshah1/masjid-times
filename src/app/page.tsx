...
async function getJamaat() {
  const res = await fetch("http://localhost:3000/api/jamaat", {
    cache: "no-store",
  });
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <section className="rounded-2xl bg-white p-6 shadow">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <TimeRow
              label="Fajr"
              adhan={fmtTime(adhan.fajr)}
              jamaat={jamaat.fajr}
            />
            <TimeRow
              label="Sunrise"
              adhan={fmtTime(adhan.sunrise)}
            />
            <TimeRow
              label="Dhuhr"
              adhan={fmtTime(adhan.dhuhr)}
              jamaat={jamaat.dhuhr}
            />
            <TimeRow
              label="Asr"
              adhan={fmtTime(adhan.asr)}
              jamaat={jamaat.asr}
            />
            <TimeRow
              label="Maghrib"
              adhan={fmtTime(adhan.maghrib)}
              jamaat={jamaat.maghrib}
            />
            <TimeRow
              label="Isha"
              adhan={fmtTime(adhan.isha)}
              jamaat={jamaat.isha}
            />
          </div>
        </section>
